# From https://github.com/aadhithya/AdaIN-pytorch
...
from AdaIN.utils import compute_mean_std
...
class VggEncoder(nn.Module):
    def __init__(self, pretrained=True, requires_grad=False) -> None:
        super().__init__()
        if pretrained:
            vgg = VGG.vgg19(weights=VGG.VGG19_Weights.DEFAULT).features
        else:
            vgg = VGG.vgg19().features
...
class Loss:
    def __init__(self, wt_s, model):
        self.wt_s = wt_s

        self.model = model

    def __call__(self, stylized_img, style_img, t):
